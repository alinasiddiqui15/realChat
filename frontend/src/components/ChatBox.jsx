...
const ChatBox = ({ selectedUser, onBack }) => {
  ...
  return (
    <div className="flex flex-col w-full h-full bg-light-800 dark:bg-dark-800 overflow-hidden relative z-10">
      {/* Header */}
      <div className="px-6 py-4 border-b border-light-600 dark:border-dark-600 bg-light-800 dark:bg-dark-800 flex items-center justify-between shrink-0 z-10">
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar bg-light-900 dark:bg-dark-900">
        {loading ? (
          <div className="flex justify-center py-10"><Loader /></div>
        ) : (
          <div className="flex flex-col space-y-4">
            {messages.length === 0 ? (
              <div className="text-center text-gray-500 dark:text-gray-400 my-10 bg-light-800 dark:bg-dark-800 inline-block mx-auto px-6 py-2 rounded-full text-xs font-semibold shadow-sm border border-light-600 dark:border-dark-600">
              ...
              messages.map((msg, index) => {
                ...
                return (
                  <div key={msg._id || index} className={`flex ${isMe ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                    <div 
                      className={`max-w-[75%] md:max-w-[60%] rounded-[20px] px-5 py-3 shadow-sm ${
                        isMe 
                          ? 'bg-primary-500 text-gray-800 dark:text-gray-100 rounded-tr-sm' 
