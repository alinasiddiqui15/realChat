...
const Login = () => {
  ...
  const handleSocialLogin = (platform) => {
    toast(`Sign in with ${platform} is simulated. Backend integration required!`, {
      icon: '🔐',
    });
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-light-900 dark:bg-dark-900 px-4 transition-colors relative overflow-hidden">
      ...
      <div className="nexus-bg-blob w-[450px] h-[450px] bg-primary-500 top-[-8%] left-[-8%]"></div>
      <div className="nexus-bg-blob w-[550px] h-[550px] bg-accent-500 bottom-[-15%] right-[-8%]"></div>

      <div className="w-full max-w-sm bg-light-800 dark:bg-dark-800 border border-light-600 dark:border-dark-600 p-8 rounded-[42px] z-10 shadow-xl shadow-gray-200/40 dark:shadow-black/30 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="text-right">
            <button
              type="button"
              onClick={() => handleSocialLogin('Forgot Password')}
              className="text-xs font-semibold text-gray-500 dark:text-gray-400 hover:text-primary-500 dark:hover:text-primary-400 transition-colors"
            >
              Forgot password?
            </button>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 py-3 bg-light-900 dark:bg-dark-900 border border-light-600/50 dark:border-dark-600/50 rounded-xl hover:bg-light-700 dark:hover:bg-dark-700 transition-colors text-xs font-bold text-gray-700 dark:text-gray-200 shadow-sm"
          >
